...
export default function ProjectsHolder({ data }) {
  return (
    <div className='py-32 h-max md:h-screen '>
        <h1 className='text-center text-2xl md:text-4xl p-4 text-gradient font-semibold'>Dünya çapındaki müşterilerimize hazrıladığımız projelerimiz</h1>
        <div className='grid grid-cols-1 md:grid-cols-2 p-4 gap-4 '>        
            ...
            return(
                <div
                key={i}
                className='p-2 sm:p-4 md:p-8 flex flex-col gap-2 text-center justify-center rounded-lg shadow-xl z-10 w-5/6 mx-auto bg-[--light]'
                >
                <Image 
                    src={`/${item.url}.png`}    
                    width={320}
